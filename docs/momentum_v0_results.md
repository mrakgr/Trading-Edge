...
| **`min`(b_pf, r_pf)** | ✅ YES | "you're only as strong as your weaker signal." Clean monotonic ladder: 1.37 → 1.49 → 1.44 → 2.07 → **2.81** (the one wobble, 1.49→1.44 between two near-equal scores, is 0.06 = noise). `min` is an **AND gate**: the top score is reachable *only* by (b3,r3), so its elite bucket is **pure** (533 trades, undiluted) → PF 2.81. One-sided trades correctly sink to the floor (the weak signal pulls them down). |
