module TradingEdge.LowFader.Program

open System
open System.Diagnostics
open Argu
open TradingEdge.LowFader.Backtest
...
type Args =
    ...
    | Ord_Volat_Lo of float
    | Ord_Volat_Hi of float
    | Ord_Max_Eff_Ewma_10m of float
    | Ord_Min_Lows_600 of int
    | Ord_Min_Rate_600 of float
    | Ord_Min_Rr of float
    ...
    interface IArgParserTemplate with
        member this.Usage =
            ...
            | Ord_Volat_Lo _ -> "SPEC-ORDINAL gate (record-only, 2026-09-05): volat_20m > this. Default 0.003 (WIDE-VG 30bp floor, §L22)."
            ...
            | Ord_Min_Rate_600 _ -> "SPEC-ORDINAL gate: lows/bars over the 600 leg >= this. Default 0.15."
            ...
            | Ord_Max_Gap_Adj_60 _ -> "SPEC-ORDINAL gate: gap_adj_60 <= this. Default 60 (WIDE-VG, §L22)."
            ...
            | Ord_Max_Dv_0945_Tape _ -> "SPEC-ORDINAL gate: dv_0945_tape < this. Default 2e7 (§L20, the $20M ceiling)."
...
[<EntryPoint>]
let main argv =
    let parser = ArgumentParser.Create<Args>(programName = "TradingEdge.LowFader")
    let parsed =
        ...
        { d with
            Intraday =
                { d.Intraday with
                    ...
                    OrdVolatLo       = parsed.GetResult(Ord_Volat_Lo,        defaultValue = d.Intraday.OrdVolatLo)
                    OrdVolatHi       = parsed.GetResult(Ord_Volat_Hi,        defaultValue = d.Intraday.OrdVolatHi)
                    OrdMaxEffEwma10m = parsed.GetResult(Ord_Max_Eff_Ewma_10m, defaultValue = d.Intraday.OrdMaxEffEwma10m)
                    OrdMinLows600    = parsed.GetResult(Ord_Min_Lows_600,    defaultValue = d.Intraday.OrdMinLows600)
                    OrdMinRate600    = parsed.GetResult(Ord_Min_Rate_600,    defaultValue = d.Intraday.OrdMinRate600)
                    ...
                    OrdMaxDv0945Tape = parsed.GetResult(Ord_Max_Dv_0945_Tape, defaultValue = d.Intraday.OrdMaxDv0945Tape)
