module TradingEdge.MaxRiderV1.Backtest

open System
open System.Globalization
open System.Collections.Generic
open DuckDB.NET.Data
open TradingEdge.MaxRiderV1.Intraday
...
let defaultConfig =
    { Intraday =
        { EntryHighWindow = 20         // SHORT the 20m high of closes (the mirror of V6's 20m low).
          ...
          MinHighsIntoLeg = 0            // ⭐ 0 = the SAMPLER (take every high). The mirror of V6 F3's
                                         // "wait for the Kth low" — untested on the short side.
...
type Trip =
    { Symbol: string
      ...
      IsNewSessHigh: bool        // ⭐ entry is also a new SESSION close-high
...
let private toTrip (c: Candidate) (notional: float) (pos: IntradayPosition) : Trip =
    ...
    | ExitedAt (exitMin, exitPx, reason) ->
        ...
        { Symbol = c.Ticker
          ...
          BarsSinceFirstHigh = pos.BarsSinceFirstHigh
          HighsSinceFirstHigh = pos.HighsSinceFirstHigh
          VwapAtEntry = pos.VwapAtEntry
          DistVwap = pos.DistVwap
          DistVwapZ = pos.DistVwapZ
          DistVwapZLog = pos.DistVwapZLog
          IsNewSessHigh = pos.IsNewSessHigh
          ...
          ExitPrice = exitPx
          ExitReason = reason
          // ⭐ SHORT: profit when the price FALLS. The sign flip vs V6 — the single most
          // corrupting thing to get wrong, since every downstream table would silently invert.
          RetMoc = (if pos.EntryPx > 0.0 then 1.0 - exitPx / pos.EntryPx else nan)
          DayClose = c.DayClose
          CloseFwd1d = c.CloseFwd1d
          CloseFwd3d = c.CloseFwd3d
          CloseFwd5d = c.CloseFwd5d
          Qty = qty
          NetPnL = qty * (pos.EntryPx - exitPx)   // ⭐ SHORT (entry - exit), not (exit - entry)
...
let header =
    ...
    + "vwap_at_entry,dist_vwap,dist_vwap_z,dist_vwap_z_log,is_new_sess_high,prev_sess_vol_high,is_new_sess_vol_high,vol_z_log,vol_z_lin,ema_at_entry,pct_chg_since_open,bar_pct,chg_1d,chg_3d,"
...
let private row (t: Trip) =
    ...
    String.Join(",",
        [| t.Symbol
           ...
           f t.VwapAtEntry; f t.DistVwap; f t.DistVwapZ; f t.DistVwapZLog; (if t.IsNewSessHigh then "1" else "0"); f t.PrevSessVolHigh; (if t.IsNewSessVolHigh then "1" else "0"); f t.VolZLog; f t.VolZLin; f t.EmaAtEntry
