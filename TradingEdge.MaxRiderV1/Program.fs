module TradingEdge.MaxRiderV1.Program

open System
open System.Diagnostics
open Argu
open TradingEdge.MaxRiderV1.Backtest
...
type Args =
    ...
    | Entry_High_Window of int
    | Exit_Low_Window of int
    | Require_Below_Vwap
    // ----- the one surviving gate -----
    | Min_Highs_Into_Leg of int
    ...
    interface IArgParserTemplate with
        member this.Usage =
            ...
            | Entry_High_Window _ -> "⭐ ENTRY: SHORT when the close makes a new N-minute HIGH of 1m-bar CLOSES. Default 20."
            | Exit_Low_Window _ -> "⭐ COVER: buy back when the close reaches the N-minute LOW of 1m-bar CLOSES. Default 5 (V6 F16/F17 found the fast target dominates on the LONG side — re-measure here)."
            | Require_Below_Vwap -> "GATE on `close < VWAP` at entry (the mirror of V6's above-VWAP gate). Default OFF — the sampler RECORDS dist_vwap so BOTH sides are sampled and the question is settled post-hoc (V6 F5/F6: distance from VWAP in EITHER direction is the real signal; PF is WORST at VWAP)."
            | Min_Highs_Into_Leg _ -> "⭐ WAIT FOR THE Kth HIGH: only SHORT once this up-leg has made >= K consecutive new highs, take ONE position per leg. 0 (default) = the sampler. The mirror of V6 F3, where waiting for the Kth LOW raised the long book PF 1.255 -> 1.968. ⚠ UNTESTED on the short side — a name making its 5th new high may be a SQUEEZE, not a fade."
...
[<EntryPoint>]
let main argv =
    let parser = ArgumentParser.Create<Args>(programName = "TradingEdge.MaxRiderV1")
    let parsed =
    ...
    let cfg =
        { d with
            Intraday =
                { d.Intraday with
                    EntryHighWindow  = parsed.GetResult(Entry_High_Window, defaultValue = d.Intraday.EntryHighWindow)
                    ExitLowWindow    = parsed.GetResult(Exit_Low_Window, defaultValue = d.Intraday.ExitLowWindow)
                    RequireBelowVwap = d.Intraday.RequireBelowVwap || parsed.Contains Require_Below_Vwap
                    MinHighsIntoLeg  = parsed.GetResult(Min_Highs_Into_Leg, defaultValue = d.Intraday.MinHighsIntoLeg)
    ...
    printfn "MaxRiderV1 — INTRADAY MEAN REVERSION, ⭐ SHORT SIDE (fade the %dm high, cover the %dm low)"
        ic.EntryHighWindow ic.ExitLowWindow
    printfn "  ⚠ NO STOP + UNBOUNDED SHORT TAIL. No borrow cost / locate / SSR modelled. MEASUREMENT ONLY."
    printfn "  db          = %s" dbPath
    ...
    printfn "  ENTRY       = SHORT close >= prior %dm MAX of closes%s%s" ic.EntryHighWindow
        (if ic.RequireBelowVwap then "   AND close < VWAP" else "   (VWAP: RECORDED not gated — sampler)")
        (if ic.MinHighsIntoLeg > 0 then sprintf "   AND >= %d highs into the leg (ONE position/leg)" ic.MinHighsIntoLeg else "")
    printfn "  EXIT        = COVER close <= prior %dm MIN of closes (target)  |  MOC    [⚠ NO STOP]" ic.ExitLowWindow
    printfn "  gate        = %s" (
