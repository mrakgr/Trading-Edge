module TradingEdge.OpeningDriverV2.Backtest
...
// ===========================================================================
// OpeningDriverV2 backtest wiring.
//
...
// bars into the IntradaySystem (Intraday.fs), which ARMS/DISARMS once per day: it
// fires the FIRST bar that passes the settled gates as one opening-drive trip (held
// to MOC or a 9-EMA stop). Same two-pipeline shape, minus the daily engine.
// ===========================================================================

/// OpeningDriverV2 config = the intraday engine knobs + the settled gates + notional.
type Config =
    { Intraday: IntradayConfig
      Notional: float }

/// Default = the settled headline book (F1–F24): scan [09:45, 10:30], fire the FIRST
/// bar with chg_1d>=0.20 & chg_3d∈[0,1.5] & log_atr>=0.013 & stop_dist>=0.03 & bl<15 &
/// bh>=1, and (skip-filter default) vol_slope>=0. One position per drive-day, held to
/// MOC or the 9-EMA-below-sess-ema-low stop. Features fold from 09:30.
let defaultConfig =
    { Intraday =
        { VolWindow = 20
          ...
          MinChg1d       = 0.20          // up >=20% on the day (the strongest single cut).
          MinChg3d       = 0.0           // 3-day-trend band floor (reject dead-cat bounce).
...
/// Run the whole OpeningDriverV2 backtest: read candidates (pipeline 1), then the
/// intraday arm/disarm engine per candidate day (pipeline 2, grouped by date so
