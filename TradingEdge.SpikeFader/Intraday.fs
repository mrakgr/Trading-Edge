...
type FlushPosition =
    { SignalSec: int             // the gate bar (features captured here)
      ...
      // ⭐ 2026-09-06 (LowFader §L16 port, user): rr-QUALIFIED leg-event counts — leg events (new 20m highs) in the CURRENT
      ...
      HighsRr1_120: int; HighsRr2_120: int; HighsRr3_120: int
      HighsRr1_180: int; HighsRr2_180: int; HighsRr3_180: int
...
type IntradaySystem(cfg: IntradayConfig, ticker: string, day: DateOnly) =
    ...
    // ⭐ S47 (user 2026-09-04): {30m,40m,1h}-low cover marks — the S37f exit grid stopped at 20m
    let minL1800 = MinMa 1800
    let minL2400 = MinMa 2400
    ...
    let mutable sMinL2400 : float voption = ValueNone
    ...
    member this.Process (bar: SecBar) =
        ...
        let prevBrL2400 = brLoL2400.BarsSinceBreach
        ...
        brLoL2400.Step()
        if breachedLo sMinL2400 then brLoL2400.OnBreach()
        brLoL3600.Step()
        ...
        for i in 0 .. active.Count - 1 do
            ...
            let struct (hiL2400, scL2400, mcL2400) = auxStep p.AuxLoL2400 p.AuxSecL2400 p.AuxMocL2400 prevBrL2400
            ...
            let state =
                ...
                | s -> s
            // ⭐ ONE record copy per bar per position (2026-09-04; the MaxFader finding):
            // this loop rebuilt the ~600-field record FIVE times per bar (fwd, aux, MA,
            // BarsHeld, State) for every open position — on an mc=0 sampler with tens
            ...
            // so a trip exited at its 9m cover retired once the +1200s forward mark filled
            // and its wider lo marks stayed NaN: aux_lo_1200 filled on only 74% of trips,
            // aux_lo_1800 on 39% — and the post-hoc fallback then substituted the trip's
            // OWN 9m exit for the missing wide mark (the S37f contamination, one layer up).
            // Positions now live to the close; OpenCount counts Holding only, so HasSlot
            // and the trip set are unchanged (zero-diff on the trip keys).
            ...
            | ExitedAt _ when not (Double.IsNaN p.FwdVwap1200)
                              && not (Double.IsNaN p.AuxLo60)
                              && not (Double.IsNaN p.AuxLo120)
                              && not (Double.IsNaN p.AuxLo300)
                              && not (Double.IsNaN p.AuxLo600)
                              && not (Double.IsNaN p.AuxLo1200)
                              && not (Double.IsNaN p.AuxLoL1800)
                              && not (Double.IsNaN p.AuxLoL2400)
                              && not (Double.IsNaN p.AuxLoL3600)
                              && not (Double.IsNaN p.AuxLo180)
                              && not (Double.IsNaN p.AuxLo240)
                              && not (Double.IsNaN p.AuxLo360)
                              && not (Double.IsNaN p.AuxLo420)
                              && not (Double.IsNaN p.AuxLo480)
                              && not (Double.IsNaN p.AuxLo540)
                              && not (Double.IsNaN p.AuxLo660)
                              && not (Double.IsNaN p.AuxLo720)
                              && not (Double.IsNaN p.AuxLo780)
                              && not (Double.IsNaN p.AuxLo840)
                              && not (Double.IsNaN p.AuxLo900)
                              && not (Double.IsNaN p.AuxLo960)
                              && not (Double.IsNaN p.AuxLo1020)
                              && not (Double.IsNaN p.AuxLo1080)
                              && not (Double.IsNaN p.AuxLo1140)
            ...
            pendingEntry <-
                ValueSome
                    { SignalSec = bar.etSec
                      ...
                      HighsRr1_120 = rr120.C1; HighsRr2_120 = rr120.C2; HighsRr3_120 = rr120.C3
                      HighsRr1_180 = rr180.C1; HighsRr2_180 = rr180.C2; HighsRr3_180 = rr180.C3
                      ...
                      AuxLoL2400 = nan; AuxSecL2400 = -1; AuxMocL2400 = false
        ...
        if brLo300.BarsSinceBreach = 0 then counters300.Reset(); rr300.Reset()
        if brLo600.BarsSinceBreach = 0 then counters600.Reset(); rr600.Reset()
        if brLo30.BarsSinceBreach = 0 then counters30.Reset()
        if brLo60.BarsSinceBreach = 0 then counters60.Reset()
        if brLo120.BarsSinceBreach = 0 then counters120.Reset(); rr120.Reset()
        if brLo180.BarsSinceBreach = 0 then counters180.Reset(); rr180.Reset()
