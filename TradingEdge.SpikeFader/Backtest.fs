module TradingEdge.SpikeFader.Backtest
...
open TradingEdge.SpikeFader.Intraday

// ===========================================================================
// SpikeFader backtest wiring (template: DipRiderV6/Backtest.fs via SurgeRider).
//
...
/// SpikeFader config = the intraday engine knobs + notional + the daily floor.
type Config =
...
CREATE TABLE trips (
    ...
    highs_rr1_120 INTEGER, highs_rr2_120 INTEGER, highs_rr3_120 INTEGER,
    highs_rr1_180 INTEGER, highs_rr2_180 INTEGER, highs_rr3_180 INTEGER,
    ...
    -- ⭐ S47: {30m,40m,1h}-low cover marks
    aux_lo_1800_px DOUBLE, aux_lo_1800_sec INTEGER, aux_lo_1800_moc BOOLEAN,
    aux_lo_2400_px DOUBLE, aux_lo_2400_sec INTEGER, aux_lo_2400_moc BOOLEAN,
    ...
    ols_slope_since_low DOUBLE, ols_r_since_low DOUBLE, bars_since_low INTEGER,
    ols_slope_since_flow DOUBLE, ols_r_since_flow DOUBLE,
    eff_since_low DOUBLE, eff_since_flow DOUBLE,
    eff9_since_low DOUBLE, eff9_since_flow DOUBLE, slots_since_low INTEGER, slots_since_flow INTEGER,
    z_since_low DOUBLE, z_since_flow DOUBLE, chan_hi_prev DOUBLE,
    chan_hi_prev_120 DOUBLE, lo_120 DOUBLE, chan_hi_prev_180 DOUBLE, lo_180 DOUBLE,
    d_lo_flow DOUBLE, ols_slope_lo_flow DOUBLE, ols_r_lo_flow DOUBLE, eff_lo_flow DOUBLE,
    arm_lo_eff_20m DOUBLE, arm_lo_eff_10m DOUBLE, arm_lo_slots_20m INTEGER, arm_lo_slots_10m INTEGER, first_high_vwap DOUBLE,
...
type TripSink(outDir: string) =
    ...
    member _.Add (c: Candidate) (notional: float) (p: FlushPosition) =
        ...
        | ExitedAt (exitSec, lastBarPx, rawReason) ->
            ...
            // 🔄 SHORT (user directive): NEVER hold a short overnight by default — the gap
            // is UNBOUNDED (the ShortSnoozer asymmetry). open_p1 stays recorded; study
            // the overnight post-hoc before any adoption.
            let toNextOpen = false
            let exitPx =
            ...
            let pnl = qty * (p.EntryPx - exitPx)   // 🔄 SHORT: profit when price FALLS
            ...
            i p.HighsRr1_120; i p.HighsRr2_120; i p.HighsRr3_120
            i p.HighsRr1_180; i p.HighsRr2_180; i p.HighsRr3_180
            ...
            f p.AuxLoL2400; auxSec p.AuxSecL2400; b p.AuxMocL2400
            ...
            f (if p.EntryPx > 0.0 then (p.EntryPx - exitPx) / p.EntryPx else nan)
            i p.BarsHeld
            ...
            f p.OlsSlopeSinceLow; f p.OlsRSinceLow; i p.BarsSinceLow
            f p.OlsSlopeSinceFlow; f p.OlsRSinceFlow
            f p.EffSinceLow; f p.EffSinceFlow
            f p.Eff9SinceLow; f p.Eff9SinceFlow; i p.SlotsSinceLow; i p.SlotsSinceFlow
            f p.ZSinceLow; f p.ZSinceFlow; f p.ChanHiPrev
            f p.ChanHiPrev120; f p.Lo120; f p.ChanHiPrev180; f p.Lo180
            f p.DLoFlow; f p.OlsSlopeLoFlow; f p.OlsRLoFlow; f p.EffLoFlow
            f p.ArmLoEff20m; f p.ArmLoEff10m; i p.ArmLoSlots20m; i p.ArmLoSlots10m; f p.FirstHighVwap
...
/// Run the whole SpikeFader sampler: candidates from trading.db (pipeline 1),
