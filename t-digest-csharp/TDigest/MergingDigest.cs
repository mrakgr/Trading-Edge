...
namespace TDigest {
    ...
    public class MergingDigest : AbstractTDigest {
        ...
        // Per-instance Random used for QuickSort pivot selection during Merge.
        // Optional; when null, Sorts falls back to a thread-local default Random.
        // Set via the constructor (sortRng parameter); a seeded Random makes
        // Merge fully deterministic, and each instance owns its own Random so
        // concurrent digests are thread-safe.
        public readonly Random SortRng;
         ...
         * @param sortRng     Optional Random used for QuickSort pivot selection during
         *                    Merge. Passing a seeded Random makes Merge fully deterministic;
         *                    each instance owns its own Random so concurrent digests are
         *                    safe. When null, Sorts falls back to a thread-local default.
         */
        public MergingDigest(double compression, Random sortRng = null) :
            this(compression, -1, -1, sortRng) { }
         ...
         * @param sortRng     Optional Random for pivot selection (see other constructor).
         */
        public MergingDigest(double compression, int bufferSize, Random sortRng = null) :
            this(compression, bufferSize, -1, sortRng) {
         ...
         * @param sortRng     Optional Random for pivot selection (see other constructor).
         */
        public MergingDigest(double compression, int bufferSize, int size, Random sortRng = null) {
            SortRng = sortRng;
        ...
        private void Merge(double[] incomingMean, double[] incomingWeight, int incomingCount,
            ...
            Sorts.Sort(incomingOrder, incomingMean, 0, incomingCount, SortRng);
