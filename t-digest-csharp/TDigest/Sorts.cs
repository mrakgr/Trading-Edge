...
namespace TDigest {
    public class Sorts {
        // Thread-local default Random so that when no explicit Random is
        // supplied, concurrent callers each get their own instance and the
        // shared state is never touched by multiple threads. (System.Random
        // is NOT thread-safe — concurrent access can corrupt its internal
        // state or return garbage.)
        private static readonly ThreadLocal<Random> DefaultPrng =
            new ThreadLocal<Random>(() => new Random());

        /// <summary>
        /// Reseed the thread-local default Random on the CURRENT thread.
        /// Does not affect other threads. For full determinism across
        /// threads, pass an explicit Random to Sort instead.
        /// </summary>
        public static void SetSeed(int seed) {
            DefaultPrng.Value = new Random(seed);
        }
        ...
        public static void Sort(int[] order, double[] values) {
            Sort(order, values, 0, values.Length, null);
        }
        ...
        public static void Sort(int[] order, double[] values, int start, int n) {
            Sort(order, values, start, n, null);
        }

        /**
         * Quick sort using an index array, with an optional user-supplied
         * Random for pivot selection. Passing an explicit Random makes the
         * sort both reproducible (given a fixed seed) and thread-safe
         * (each caller owns its own Random).
         *
         * @param order  The pre-allocated index array
         * @param values The values to sort
         * @param start  The first element to sort
         * @param n      The number of values to sort
         * @param rng    Optional Random for pivot selection; when null, the
         *               thread-local default Random is used.
         */
        public static void Sort(int[] order, double[] values, int start, int n, Random rng) {
            for (var i = start; i < start + n; i++) {
                order[i] = i;
            }

            var effectiveRng = rng ?? DefaultPrng.Value;
            QuickSort(order, values, start, start + n, 64, effectiveRng);
         ...
         * @param rng    Random used for pivot selection.
         */
        private static void QuickSort(int[] order, double[] values, int start, int end, int limit, Random rng) {
            // the while loop implements tail-recursion to avoid excessive stack calls on nasty cases
            while (end - start > limit) {

                // pivot by a random element
                var pivotIndex = start + rng.Next(end - start);
...
//            checkPartition(order, values, pivotValue, start, low, high, end);
                ...
                if (low - start < end - high) {
                    QuickSort(order, values, start, low, limit, rng);
                    ...
                    QuickSort(order, values, high, end, limit, rng);
                    // this is really a way to do
