module TradingEdge.SurgeRiderV2.Intraday
...
//   ENTRY SIGNAL (⭐ V2 PULLBACK+CONFIRMATION, user 2026-07-27; P3: the
//                 falling touch has NEGATIVE 1m drift — buy the TURN, not the
//                 knife). Three-step state machine:
//                   ARM:    TREND-UP on the entry channel (bars-since-LOW-
//                           breach > bars-since-HIGH-breach; bsl = -1 with
//                           bsh >= 0 qualifies) AND the vwap CROSSES BELOW
//                           lo*(1-alpha) + hi*alpha of the STRICTLY-PRIOR
//                           N-bar channel (alpha = EntryAlpha, 0.5 = mid).
//                   ENTER:  while armed, the vwap exceeds the STRICTLY-PRIOR
//                           ConfirmBars-bar MAX (the micro-breakout out of
//                           the dip). One entry per arm.
//                   DISARM: entry fires. A LOW breach does NOT disarm —
//                           arm_lo_breaches records it (0 = the strict
//                           higher-low rule; >=1 = V-washout turn) for the
//                           post-hoc split. Re-arming needs the vwap back
//                           above the level and a fresh cross below it.
//                 Same hard floors as V1 (60-bar dollar-volume AND trade
//                 count), inside the entry window.
...
type SurgePosition =
    { SignalSec: int             // the gate bar (features captured here)
      ...
      Breach60: int
      Breach30: int
      // ----- ⭐ V2: the LOW-side breach mirror (bsl per channel; -1 = never).
      // The 1200 one is BarsSinceLow1200 below (the leg-age counter records
      // the same quantity). -----
      BreachLoSess: int
      BreachLo300: int
      BreachLo120: int
      ...
      ChanHi: float              // ⭐ V2: the STRICTLY-PRIOR ENTRY-channel max/min at the signal.
      ChanLo: float              // pos-in-range = (signal_vwap-chan_lo)/(chan_hi-chan_lo) in SQL
      // ----- ⭐ the ARM context (pullback+confirmation): the touch that armed
      // this signal. Depth-of-dip = ln(arm_vwap/arm_min_vwap); turn latency =
      // arm_bars present bars from touch to confirmation. -----
      ArmSec: int                // etSec of the arming touch bar
      ArmVwap: float             // vwap at the touch
      ArmMinVwap: float          // the LOWEST vwap seen while armed (incl. the touch bar)
      ArmBars: int               // present bars from the touch to this signal
      ArmLoBreaches: int         // entry-channel LOW breaches during the arm episode
                                 // (0 = strict higher-low; >=1 = V-washout turn)
...
type IntradayConfig =
    { EntryChannelBars: int      // ⭐ ENTRY: vwap > the prior N-present-bar MAX of vwaps. Default 300
      ...
      MinAbsEff20m: float
      // ⭐ V2 PULLBACK ALPHA (user, 2026-07-27): the entry level is
      // lo*(1-alpha) + hi*alpha of the strictly-prior entry channel. 0.5 = the
      // midpoint; smaller = a deeper pullback is demanded before entry.
      EntryAlpha: float
      // ⭐ THE CONFIRMATION CHANNEL (user, 2026-07-27): while armed, enter when
      // the vwap exceeds the strictly-prior N-bar max — the turn out of the
      // dip. One of {20,25,30,45,60,120}. Default 30.
      ConfirmBars: int
...
type IntradaySystem(cfg: IntradayConfig, ticker: string, day: DateOnly) =
    ...
    let entryMin = chanMin cfg.EntryChannelBars     // ⭐ V2: the pullback level needs both sides
    let confirmMax = chanMax cfg.ConfirmBars        // ⭐ the turn detector (pullback+confirmation)
    ...
    // ----- ⭐ V2: the LOW-side breach mirror (bsl per channel; 1200 = legLow) -----
    ...
    // the entry-channel bsh/bsl pair for the trend test (Program validates the
    // window to one of {30,60,120,300,1200})
    let entryBrHi =
        match cfg.EntryChannelBars with
        | 30 -> br30 | 60 -> br60 | 120 -> br120 | 300 -> br300 | 1200 -> br1200
        | n -> invalidArg "EntryChannelBars" $"no breach counter for {n}"
    let entryBrLo =
        match cfg.EntryChannelBars with
        | 30 -> brLo30 | 60 -> brLo60 | 120 -> brLo120 | 300 -> brLo300 | 1200 -> legLow
        | n -> invalidArg "EntryChannelBars" $"no low breach counter for {n}"
    ...
    // ⭐ V2 edge trigger: was the PREVIOUS bar already below the pullback level?
    let mutable prevBelowLevel = false
    // ⭐ the ARM state (pullback+confirmation)
    let mutable armed = false
    let mutable armSec = -1
    let mutable armVwap = nan
    let mutable armMinVwap = nan
    let mutable armBars = 0
    let mutable armLoBreaches = 0
    ...
    member this.Process (bar: SecBar) =
        ...
        let priorEntryMin =
            match cfg.EntryChannelBars with
            | 30 -> sMin30 | 60 -> sMin60 | 120 -> sMin120 | 300 -> sMin300 | 1200 -> sMin1200
            | _ -> ValueNone
        let priorConfirmMax =
            match cfg.ConfirmBars with
            | 20 -> sMax20 | 25 -> sMax25 | 30 -> sMax30 | 45 -> sMax45 | 60 -> sMax60 | 120 -> sMax120
            | _ -> ValueNone
        ...
        // ⭐ V2: the low-side mirror — a bar under the strictly-prior channel MIN
        let breachedLo (prior: float voption) = match prior with ValueSome lo -> bar.vwap < lo | ValueNone -> false
        brLoSess.Step(); brLo30.Step(); brLo60.Step(); brLo120.Step(); brLo300.Step()
        ...
        // ⭐ V2 PULLBACK+CONFIRMATION (P3). ARM on the touch: trend-up (the
        // channel's last event was a HIGH breach; bsl = -1 with a live bsh
        // also qualifies) AND the vwap crosses below the alpha-blend of the
        // strictly-prior channel. ENTER on the turn: while armed, the vwap
        // exceeds the strictly-prior ConfirmBars-bar max. DISARM on entry or
        // on an entry-channel LOW breach (the pullback became a breakdown).
        let trendUp =
            entryBrHi.BarsSinceBreach >= 0
            && (entryBrLo.BarsSinceBreach < 0
                || entryBrLo.BarsSinceBreach > entryBrHi.BarsSinceBreach)
        let belowLevel =
            match priorEntryMax, priorEntryMin with
            | ValueSome hi, ValueSome lo ->
                bar.vwap < lo * (1.0 - cfg.EntryAlpha) + hi * cfg.EntryAlpha
            | _ -> false
        // ⚠ a low breach does NOT disarm (user question, 2026-07-27): the
        // washout-turn population stays IN and post-hoc splits on
        // arm_lo_breaches (0 = the strict higher-low rule; >=1 = V-washouts).
        // An arm can never START on a breach bar — bsl=0 kills trendUp.
        if trendUp && belowLevel && not prevBelowLevel then
            armed <- true
            armSec <- bar.etSec
            armVwap <- bar.vwap
            armMinVwap <- bar.vwap
            ...
            armBars <- armBars + 1
            armMinVwap <- min armMinVwap bar.vwap
            if entryBrLo.BarsSinceBreach = 0 then armLoBreaches <- armLoBreaches + 1
        prevBelowLevel <- belowLevel
        // the confirmation: the turn out of the dip (cannot fire on the arm
        // bar — the touch is BELOW the level, which sits under the recent max)
        let isConfirm =
            armed
            && confirmMax.Count = confirmMax.WindowSize
            && (match priorConfirmMax with ValueSome m -> bar.vwap > m | ValueNone -> false)
        if isConfirm then armed <- false
        let floorsOk =
        ...
        let effOk =
            ...
            || (match slotLag.Last, slotLag.Lagged, slotAbsSum.State with
                ...
                | _ -> false)
        if inWindow && channelWarm && isConfirm && floorsOk && volOk && effOk && this.HasSlot then
            pendingEntry <-
                ValueSome
                    { SignalSec = bar.etSec
                      SignalVwap = bar.vwap
                      ...
                      ArmSec = armSec
                      ArmVwap = armVwap
                      ArmMinVwap = armMinVwap
                      ArmBars = armBars
                      ArmLoBreaches = armLoBreaches
                      Gap60 = gap60.Gaps
                      Gap30 = gap30.Gaps
                      Gap15 = gap15.Gaps
