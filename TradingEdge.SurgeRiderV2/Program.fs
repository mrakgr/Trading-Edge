module TradingEdge.SurgeRiderV2.Program

open System
open System.Diagnostics
open Argu
open TradingEdge.SurgeRiderV2.Backtest
...
let private defaultOutDir = "/tmp/surgeriderv2_trips"
...
type Args =
    ...
    interface IArgParserTemplate with
        member this.Usage =
            ...
            | Min_Abs_Eff_20m _ -> "⭐ HARD EFF GATE (F31b): |eff_20m| >= this at the signal (cold eff FAILS a positive floor). Default 0 = off. 0.40 = the V2 eff-hi bake-in (~16x fewer trips)."
            | Entry_Alpha _ -> "⭐ PULLBACK LEVEL: the touch of lo*(1-alpha)+hi*alpha of the strictly-prior entry channel (trend-up) ARMS the state. Default 0.5 = the midpoint; smaller = deeper pullback."
            | Confirm_Bars _ -> "⭐ THE CONFIRMATION: while armed, enter when the vwap exceeds the strictly-prior N-bar MAX (the turn out of the dip). One of {20,25,30,45,60,120}. Default 30."
...
[<EntryPoint>]
let main argv =
    let parser = ArgumentParser.Create<Args>(programName = "TradingEdge.SurgeRiderV2")
    let parsed =
        ...
        { d with
            Intraday =
                { d.Intraday with
                    ...
                    EntryAlpha       = parsed.GetResult(Entry_Alpha,        defaultValue = d.Intraday.EntryAlpha)
                    ConfirmBars      = parsed.GetResult(Confirm_Bars,       defaultValue = d.Intraday.ConfirmBars)
    ...
    // V2: the entry channel also drives the bsh/bsl trend counters — only the
    // windows with a low-side breach mirror qualify.
    let entryChanSet = [ 30; 60; 120; 300; 1200 ]
    ...
    if not (List.contains cfg.Intraday.ExitChannelBars chanSet) then
        ...
        exit 1
    let confirmSet = [ 20; 25; 30; 45; 60; 120 ]
    if not (List.contains cfg.Intraday.ConfirmBars confirmSet) then
        eprintfn "FATAL: --confirm-bars %d — must be one of %A." cfg.Intraday.ConfirmBars confirmSet
        exit 1
    ...
    printfn "SurgeRiderV2 — 1s PULLBACK+CONFIRM (trend-up %d-bar chan, arm at alpha=%.2f, enter on %d-bar-high reclaim; LONG)" ic.EntryChannelBars ic.EntryAlpha ic.ConfirmBars
    printfn "  db          = %s" dbPath
    ...
    printfn "  ENTRY       = ARM: bsl>bsh %d-bar chan, vwap under lo+%.2f*(hi-lo);  ENTER: vwap > prior %d-bar MAX   AND dv60 >= $%.0fk AND tc60 >= %.0f   (fill: NEXT bar)"
        ic.EntryChannelBars ic.EntryAlpha ic.ConfirmBars (ic.DvFloor60 / 1e3) ic.TcFloor60
