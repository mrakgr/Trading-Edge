module TradingEdge.SurgeRiderV2.Backtest

open System
open System.Collections.Generic
open DuckDB.NET.Data
open TradingEdge.SurgeRiderV2.Intraday
...
type Config =
    { Intraday: IntradayConfig
      ...
      MinRvol0945: float
      /// ⭐ V2 (user, 2026-07-25): universe gate on the PRIOR day's close in day-D's raw
      /// (post-split) scale — prev_adj_close / adj_ratio >= this. Knowable BEFORE the open
      /// (D-1 close), unlike entry price. 0 = off. 2.0 = the V2 ">=$2 stocks" universe
      /// (S8b/S8c: sub-$1 is priced out on every EU-accessible broker route).
      MinPrevClose: float }
...
let defaultConfig =
    { Intraday =
        { EntryChannelBars = 60         // ⭐ V2: the 1m channel — trend test (bsl > bsh) + pullback
                                        // level both live on this window. {30,60,120,300,1200}.
          ExitChannelBars  = 60         // the stop: vwap < the prior 1m MIN, as usual
          ExitZBars        = 60
          ...
          EntryAlpha       = 0.5        // ⭐ V2: the pullback level = channel midpoint
          ConfirmBars      = 30         // ⭐ the turn detector: prior 30-bar max reclaim enters
...
CREATE TABLE trips (
    ...
    breach_lo_sess INTEGER, breach_lo_300 INTEGER, breach_lo_120 INTEGER,
    breach_lo_60 INTEGER, breach_lo_30 INTEGER,
    trade_idx INTEGER, bars_since_low_1200 INTEGER,
    open_at_signal INTEGER, vwap_1200 DOUBLE, chan_1200_high DOUBLE,
    chan_hi DOUBLE, chan_lo DOUBLE,
    arm_sec INTEGER, arm_vwap DOUBLE, arm_min_vwap DOUBLE, arm_bars INTEGER,
    arm_lo_breaches INTEGER,
...
type TripSink(outDir: string) =
    ...
    member _.Add (c: Candidate) (notional: float) (p: SurgePosition) =
        ...
        | ExitedAt (exitSec, exitPx, reason) ->
            ...
            i p.BreachLoSess; i p.BreachLo300; i p.BreachLo120
            i p.BreachLo60; i p.BreachLo30
            i p.TradeIdx; i p.BarsSinceLow1200
            i p.OpenAtSignal; f p.Vwap1200; f p.Chan1200High
            f p.ChanHi; f p.ChanLo
            i p.ArmSec; f p.ArmVwap; f p.ArmMinVwap; i p.ArmBars
            i p.ArmLoBreaches
