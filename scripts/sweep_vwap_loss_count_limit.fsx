...
// Same base systems as the other sweeps
...
// Losing-trade cap ladder: 5-40 losing trades/day, plus no limit as control.
// The hypothesis is that high activity is fine when it's profitable; only a
// day producing lots of *losers* in a row is the kind of day worth killing.
let maxLossesGrid : int option[] =
    [| None
       Some 5
       Some 8
       Some 12
...
let candidates =
    [| for (baseName, mkSys) in baseSystems do
         for ml in maxLossesGrid do
            let tag =
                match ml with
                | None -> "no limit"
                | Some n -> sprintf "L<=%3d " n
            { Name = sprintf "%s %s" baseName tag
              MkSystem = mkSys
              MaxLosses = ml } |]

printfn "Sweep: %d candidates × %d files (losing-trade cap only)" candidates.Length availableEntries.Length
printfn ""
    ...
    | Some trades ->
        ...
        | Some o, Some c ->
            ...
            for i in 0 .. N - 1 do
                ...
                let sys = c.MkSystem ()
                let sim =
                    match c.MaxLosses with
                    | Some n -> VwapSimulator(window, sys, positionSize, maxLosses = n)
...
// ----- 6. Grid view: systems × losing-trade caps -----
printfn ""
printfn "=== Grid: total P&L by system × losing-trade cap ==="
printf "%-18s" "system"
for ml in maxLossesGrid do
    match ml with
    | None -> printf " %10s" "no limit"
    | Some n -> printf " %10s" (sprintf "L<=%d" n)
printfn ""
