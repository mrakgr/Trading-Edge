-- Mark-to-market monthly P&L decomposition for ShortFadeMA (OrderflowShortFadeMA).
-- Short-only mirror of mtm_flowswing.sql.
--
-- Outputs:
--   data/crypto/short_fade_ma_dualcvd_gapfix_origflags/mtm_monthly.csv
...
SELECT 'shortfade' AS variant, symbol, side, entry_price, exit_price, effective_notional, fees,
       funding_pnl, net_pnl,
...
FROM read_csv_auto('data/crypto/short_fade_ma_dualcvd_gapfix_origflags/backtest_results_trips_1m_pr0.14_ma72h_cvd240m_ed0.2_short.csv');
...
SELECT
  ...
  -- short: price_start - price_end (we profit when price falls)
...
) TO 'data/crypto/short_fade_ma_dualcvd_gapfix_origflags/mtm_monthly.csv' (HEADER, DELIMITER ',');
