#!/usr/bin/env python3
"""Plot MTM equity curves for FlowSwing-long vs ShortFadeMA-mirror at the
risk-adjusted-best CVD-window cell, plus the implied combined book.

FlowSwing long: pd=0.14, ma=72h,  cvd=240m, rvol=0.75
Short cvd128h:  pr=0.14, ma=4374h, cvd=7680m, rvol=0.75
"""
...
SHORT_CSV = os.path.join(ROOT, "data/crypto/short_fade_ma_longma_cvd_sweep/mtm_monthly_pr0.14_ma4374h_cvd7680m.csv")
OUT_HTML  = os.path.join(ROOT, "data/crypto/short_fade_ma_longma_cvd_sweep/equity_curves_long_vs_cvd128h.html")
...
LEGS = [
    ("long",     "FlowSwing long (pd=14% ma=72h cvd=4h)",                  "#2ca02c"),
    ("short",    "ShortFadeMA mirror (pr=14% ma=4374h/6mo cvd=7680m/128h)","#d62728"),
...
fig.update_layout(
    title="FlowSwing long + ShortFadeMA mirror w/ 6mo MA & 128h CVD — proper monthly MTM",
...
for col, label, _ in LEGS:
    ...
    print(f"{label:<60s} total=${s.sum():>9,.0f}  max_dd=${df[f'dd_{col}'].min():>9,.0f}  wins={int((s>0).sum()):>2d} losses={int((s<0).sum()):>2d}  ann_sharpe={sharpe:.2f}")
