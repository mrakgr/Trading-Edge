#!/usr/bin/env python3
"""Plot MTM equity curves for FlowSwing (long), ShortFadeMA-mirror (short),
and the implied combined book (long + short).

Both legs use FlowSwing's parameter set (pr/pd=0.14, ma=72h, cvd=240m, rvol=0.75,
no 200h CVD overlay, single CVD MA-cross cover, no stop, ed=0.20, high8h ref).
"""
...
SHORT_CSV = os.path.join(ROOT, "data/crypto/short_fade_ma_mirror/mtm_monthly.csv")
OUT_HTML  = os.path.join(ROOT, "data/crypto/short_fade_ma_mirror/equity_curves_long_short_combined.html")
...
LEGS = [
    ("long",     "FlowSwing (long)",            "#2ca02c"),
    ("short",    "ShortFadeMA-mirror (short)",  "#d62728"),
...
fig.update_layout(
    title="FlowSwing long + ShortFadeMA mirror short — proper monthly MTM",
...
for col, label, _ in LEGS:
    total = df[col].sum()
    max_dd = df[f"dd_{col}"].min()
    win = (df[col] > 0).sum(); los = (df[col] < 0).sum()
    sharpe = df[col].mean()/df[col].std() * 12**0.5 if df[col].std() else float("nan")
    print(f"{label:<35s} total=${total:>9,.0f}  max_dd=${max_dd:>9,.0f}  wins={win:>2d} losses={los:>2d}  ann_sharpe={sharpe:.2f}")
