-- MTM for ShortFadeMA mirror at pr=0.14, ma=4374h, cvd=7680m (128h).
...
SELECT 'shortcvd128h' AS variant, symbol, side, entry_price, exit_price, effective_notional, fees,
       funding_pnl, net_pnl,
...
FROM read_csv_auto('data/crypto/short_fade_ma_longma_cvd_sweep/results_trips_1m_pr0.14_ma4374h_cvd7680m_rvol0.75_ed0.2_short.csv');
...
joined AS (
  SELECT e.*, a_s.anchor_close AS anchor_start, a_e.anchor_close AS anchor_end,
...
SELECT variant, trip_id, symbol, month,
...
) TO 'data/crypto/short_fade_ma_longma_cvd_sweep/mtm_monthly_pr0.14_ma4374h_cvd7680m.csv' (HEADER, DELIMITER ',');
