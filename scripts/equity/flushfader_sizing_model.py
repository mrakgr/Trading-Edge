...
def X_hybrid5():   # C5: gap=0 flag + LINEAR slope in gap count (per 10 gaps), log volat, flags
    cols = [np.ones(n), (gi == 0).astype(float), B.gap60.values.astype(float) / 10.0, np.log(B.volat.values.astype(float) / 100.0), ri.astype(float), ti.astype(float)]
    return np.column_stack(cols), ["1", "gap = 0", "gap / 10 (linear)", "log(volat/100)", "rate600 fast", "S tier"]
def X_dummies3():   # A3: model A with the gap bands 4-12 and 13-39 MERGED (gap 0 / 1-3 / 4-39)
    cols = [np.ones(n), (gi == 1).astype(float), (gi >= 2).astype(float)] + [(vi == k).astype(float) for k in range(1, len(VL))] + [ri.astype(float), ti.astype(float)]
    return np.column_stack(cols), ["1", "gap 1-3", "gap 4-39"] + [f"volat {VL[k]}" for k in range(1, len(VL))] + ["rate600 fast", "S tier"]
def X_slopes():
...
XA, NA = X_dummies(); XI, NI = X_dummies(inter=True); XB, NB = X_slopes(); XD, ND = X_dummies(dense_vol=True); XC, NC = X_hybrid(); XC2, NC2 = X_hybrid2(); XC3, NC3 = X_hybrid3(); XC4, NC4 = X_hybrid4(); XC5, NC5 = X_hybrid5(); XA3, NA3 = X_dummies3()

# ---- coefficients on all years (as RATIOS: exp(b) for W/L, odds-ratio for p)
for lab, X, names in [("A: band dummies", XA, NA), ("A+I: dummies + premium×fast interaction", XI, NI), ("A+D: dummies + dense×volatile interaction", XD, ND), ("B: one slope per feature", XB, NB), ("C: hybrid (gap=0 flag, log volat, two flags)", XC, NC), ("C2: C + log(1+gap) slope", XC2, NC2), ("C3: C + gap>=13 step", XC3, NC3), ("C4: gap dummies + log volat + flags", XC4, NC4), ("C5: gap=0 flag + linear gap slope", XC5, NC5), ("A3: model A with gap 4-12 and 13-39 merged", XA3, NA3)]:
...
for lab, fit, app in [("in-sample (fit all, apply all)", all_m, all_m), ("holdout: fit 2020-23 → apply 2024-26", early, late), ("holdout: fit 2024-26 → apply 2020-23", late, early)]:
    ...
    for mlab, X in [("model A (dummies)", XA), ("model A+I (dummies + premium×fast)", XI), ("model A+D (dummies + dense×volatile)", XD), ("model B (slopes)", XB), ("model C (hybrid)", XC), ("model C2 (step + slope)", XC2), ("model C3 (two steps)", XC3), ("model C4 (gap dummies + log volat)", XC4), ("model C5 (gap=0 + linear slope)", XC5), ("model A3 (dummies, gap 0 / 1-3 / 4-39)", XA3)]:
