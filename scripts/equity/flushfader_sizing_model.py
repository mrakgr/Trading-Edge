...
XA, NA = X_dummies(); XI, NI = X_dummies(inter=True); XB, NB = X_slopes(); XD, ND = X_dummies(dense_vol=True)

# ---- coefficients on all years (as RATIOS: exp(b) for W/L, odds-ratio for p)
for lab, X, names in [("A: band dummies", XA, NA), ("A+I: dummies + premium×fast interaction", XI, NI), ("A+D: dummies + dense×volatile interaction", XD, ND), ("B: one slope per feature", XB, NB)]:
...
for lab, fit, app in [("in-sample (fit all, apply all)", all_m, all_m), ("holdout: fit 2020-23 → apply 2024-26", early, late), ("holdout: fit 2024-26 → apply 2020-23", late, early)]:
    ...
    for mlab, X in [("model A (dummies)", XA), ("model A+I (dummies + premium×fast)", XI), ("model A+D (dummies + dense×volatile)", XD), ("model B (slopes)", XB)]:
