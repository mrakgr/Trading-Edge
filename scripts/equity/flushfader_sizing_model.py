...
for lab, fit, app in [("in-sample (fit all, apply all)", all_m, all_m), ("holdout: fit 2020-23 → apply 2024-26", early, late), ("holdout: fit 2024-26 → apply 2020-23", late, early)]:
    ...
    for mlab, X in [("model A (dummies)", XA), ("model A+I (dummies + premium×fast)", XI), ("model A+D (dummies + dense×volatile)", XD), ("model B (slopes)", XB), ("model C (hybrid)", XC), ("model C2 (step + slope)", XC2), ("model C3 (two steps)", XC3), ("model C4 (gap dummies + log volat)", XC4), ("model C5 (gap=0 + linear slope)", XC5)]:
