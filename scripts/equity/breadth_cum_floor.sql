...
br AS (SELECT date, LAG(pct_above_20) OVER (ORDER BY date) b20 FROM '/tmp/breadth_100k.parquet')
...
FROM t WHERE b20 >= f;
.mode box
SELECT 'b20>=0.0' g,* FROM floor(0.0) UNION ALL SELECT 'b20>=0.4',* FROM floor(0.4)
 UNION ALL SELECT 'b20>=0.5',* FROM floor(0.5) UNION ALL SELECT 'b20>=0.6',* FROM floor(0.6)
 UNION ALL SELECT 'b20>=0.65',* FROM floor(0.65) UNION ALL SELECT 'b20>=0.7',* FROM floor(0.7)
 UNION ALL SELECT 'b20>=0.75',* FROM floor(0.75) UNION ALL SELECT 'b20>=0.8',* FROM floor(0.8);
