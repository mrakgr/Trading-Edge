...
WHERE br.b_lag1>0.5 AND raw.entry_date>=DATE '2005-01-01'
  ...
  AND raw.rvol_at_entry>=1.0 AND raw.rvol_at_entry<15.0;     -- restrict to 1..15

.mode box
SELECT '== rvol [1,15) QUARTILES (move>=10, caps on) ==' z;
WITH q AS (SELECT *, NTILE(4) OVER (ORDER BY rvol_at_entry) quartile FROM t)
SELECT quartile, ROUND(MIN(rvol_at_entry),2) rvol_lo, ROUND(MAX(rvol_at_entry),2) rvol_hi, COUNT(*) n,
