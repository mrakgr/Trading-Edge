...
cd /home/mrakgr/Trading-Edge/research; START=${1:?}; END=${2:?}; TAG=${3:?}; TABLE=${4:-lowfader_whitelist}; EXTRA=${5:-}; OUT=data/lowfader_wl_$TAG; rm -rf $OUT   # EXTRA = extra engine flags, e.g. "--min-volat-20m 0.003"
FF_CANDIDATE_TABLE=$TABLE DOTNET_GCHeapHardLimitPercent=55 ./TradingEdge.LowFader/bin/Release/net10.0/TradingEdge.LowFader \
  ...
  --start-date $START --end-date $END --base-run --out-dir $OUT ${EXTRA} > data/lowfader_wl_$TAG.log 2>&1
