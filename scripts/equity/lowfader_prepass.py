"""LowFader whitelist prepass (SESSION LOW mirror) (2026-09-03). PURE-SQL, provably a SUPERSET of the
...
SEC="data/intraday_1s_slim"; OUT="data/lowfader_whitelist.parquet"
...
wc.execute("DROP TABLE IF EXISTS lowfader_whitelist")
wc.execute(f"""CREATE TABLE lowfader_whitelist AS
...
n=wc.execute("SELECT count(*), count(DISTINCT ticker), min(date), max(date) FROM lowfader_whitelist").fetchone()
tot=wc.execute("SELECT count(*) FROM mr_candidate_1s_v2").fetchone()[0]
print(f"lowfader_whitelist: {n[0]:,} tkd ({100*n[0]/tot:.1f}% of {tot:,}), {n[1]:,} tickers, {n[2]}..{n[3]}", flush=True)
