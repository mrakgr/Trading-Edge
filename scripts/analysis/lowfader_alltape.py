...
mc=0 sampler attribution with tkd counts and PF22; mc=1 replay for the spec book. Corpus = the four globs h1/h1b/h2/h2b (two OOM-killed halves + their resume spans).
...
DIRP = sys.argv[1] if len(sys.argv) > 1 else "['data/lowfader_alltape_h1/*.parquet','data/lowfader_alltape_h1c/*.parquet','data/lowfader_alltape_h1b/*.parquet','data/lowfader_alltape_h2/*.parquet','data/lowfader_alltape_h2e/*.parquet','data/lowfader_alltape_h2b/*.parquet','data/lowfader_alltape_h2c/*.parquet','data/lowfader_alltape_h2d/*.parquet']"
# CUTS: h1/h2 were OOM-killed with 14 workers in flight (partial days near the boundary; 2022-01-24 missing) -> rerun into h1c/h2e;
# h2b was stopped with 5 workers in flight -> rerun into h2c/h2d. Keep each original only BEFORE its cut.
CUTS = {'_h1/': '2022-01-19', '_h2/': '2024-12-06', '_h2b/': '2025-09-23'}
CUTW = ' AND '.join(f"NOT (t.filename LIKE '%{k}%' AND CAST(t.trade_date AS DATE) >= DATE '{v}')" for k,v in CUTS.items())
...
df = c.execute(f"""SELECT t.symbol, t.trade_date, t.signal_sec, t.entry_sec, t.exit_sec, t.entry_px, t.ret_exit AS ret, t.aux_hi_600_px, t.aux_hi_600_sec, t.breach_1200,
  ...
  FROM read_parquet({DIRP}, filename=true) t
  LEFT JOIN mr_candidate_1s_v2 m ON m.ticker=t.symbol AND m.date=CAST(t.trade_date AS DATE)
  LEFT JOIN lowfader_whitelist w ON w.ticker=t.symbol AND w.date=CAST(t.trade_date AS DATE)
  WHERE {CUTW}""").df()
...
d=df.copy(); d['r']=pd.cut(d.rr,[0,0.25,0.5,1,2,4,8,1e9]); d['l']=pd.cut(d.dollar_vol_60,[-1,150e3,250e3,500e3,1e6,2.5e6,1e12])
...
# ⚠ the engine's hard entry floors dv60>=$100k ∧ tc60>=60 were ACTIVE in this run (--base-run keeps the signal definition) — nothing below $100k/min exists here
bands(df[df.dollar_vol_60<250e3],'rr',[0,0.25,0.5,1,2,4,8,1e9],'LEAST LIQUID sampled (dv_60 in [$100k,$250k))'); bands(df[df.dollar_vol_60>=1e6],'rr',[0,0.25,0.5,1,2,4,8,1e9],'LIQUID (dv_60 >= $1M)')
