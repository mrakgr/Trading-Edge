...
def chart_day(con, symbol, date, trips, minute_dir, output_dir):
    ...
    for t in trips:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(f"ENTRY {t['entry_time']}  px %{{y:.4f}}<br>"
                           f"log_atr {fnum(t,'log_atr_20'):.4f}  vslope {fnum(t,'vol_slope_20'):.3f}<br>"
                           f"sum6 {t.get('sum_above_6','?')}  1d {fnum(t,'chg_1d')*100:+.1f}%  "
                           f"rvol5m20d {fnum(t,'rvol_5m_20d'):.0f}<extra></extra>")), row=1, col=1)
    ...
    title = (f"{symbol} {date} — {n} DipRiderV3 trip(s)  |  pnl {pnl:,.0f}<br>"
             f"<sup>log_atr {fnum(t0,'log_atr_20'):.4f}  vslope {fnum(t0,'vol_slope_20'):.3f}  "
             f"sum6 {t0.get('sum_above_6','?')}  tight {fnum(t0,'tightness_20'):.1f}  "
             f"1d {fnum(t0,'chg_1d')*100:+.1f}%  vs_vwap {fnum(t0,'entry_vs_vwap')*100:+.1f}%  "
             f"rvol5m20d {fnum(t0,'rvol_5m_20d'):.0f}  "
...
def main():
    ...
    ap.add_argument('--trips', default='/tmp/diprider_v3_2020_aplus.csv')
    ap.add_argument('--minute-dir', default='data/minute_aggs')
    ap.add_argument('--output-dir', default='data/charts/diprider_v3')
