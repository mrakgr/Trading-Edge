...
def plot_volume_bars(bars, output_html, input_csv):
    ...
    # Width-weighted bars centered on VWAP, with min-height fallback so
    # near-zero-stddev bars stay visible. Mirrors massive_volume.py.
    ...
    # min_height = min(0.01, 0.001 * session_vwap)  # disabled for synthetic data: real 4σ is sub-cent, floor was clamping every bar
    min_height = 0.0
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color='green',
        marker_line_width=0,
        width=[v * 0.8 for v in bar_volumes],
        ...
        hovertemplate='<b>Volume:</b> %{customdata[0]:,.0f}<br>' +
                      ...
                      '<b>Time:</b> %{customdata[5]:.2f}-%{customdata[6]:.2f}m<br>' +
                      '<b>Duration:</b> %{customdata[7]:.3f}s<br>' +
                      '<b>Trades:</b> %{customdata[8]}<br>' +
                      '<b>Labels:</b> %{customdata[9]}<extra></extra>'
    ), row=1, col=1)
...
if __name__ == '__main__':
    ...
    volume_per_bar = float(sys.argv[2]) if len(sys.argv) > 2 else 10000.0
    if len(sys.argv) > 3:
