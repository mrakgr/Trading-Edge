...
/// Orderflow-MA v1 wrapper. Same surface as CumsumZCell. MaWindowHours
/// slot in the reporting config carries the (integer-rounded) cumsum
/// threshold so the breakdown grouping key is unique per threshold.
type MAv1Cell(symbol: string, timeframe: string, cfg: OrderflowMAv1.MAv1Config) =
    let bucketUs = bucketUsOfTimeframe timeframe
    let builder = TimeBarBuilder(bucketUs)
    let cfgWithBucket = { cfg with OrderflowMAv1.MAv1Config.BucketUs = bucketUs }
    let engine = OrderflowMAv1.Engine(cfgWithBucket)
    let mutable barCount = 0
    let mutable startUs = 0L
    let mutable endUs = 0L
    let mutable hasAny = false
    ...
    let reportingCfg =
        { defaultConfig (int (round cfg.CumsumThreshold)) with
            Notional = cfg.Notional
            TakerFee = cfg.TakerFee
            // v1 always emits both sides; whether they have non-zero notional
            // is governed by the long/short sizing buckets, not by an
            // entry-level allow flag. Reported as AllowShort=true so the
            // existing breakdown grouping treats the trip set as long+short.
            AllowShort = true
            BucketUs = bucketUs
            MaxAdverseFraction = cfg.MaxAdverseFraction
            ReferenceVol = cfg.ReferenceVol
...
/// LongFadeMA wrapper — same pattern as ExtremeRvolLongCell. Long-only.
/// MaWindowHours field repurposed to carry the cover-MA window in hours
/// (the only natural integer dimension to bin breakdown rows by here).
type LongFadeMACell(symbol: string, timeframe: string, cfg: OrderflowLongFadeMA.LongFadeMAConfig) =
    let bucketUs = bucketUsOfTimeframe timeframe
    let builder = TimeBarBuilder(bucketUs)
    let cfgWithBucket = { cfg with OrderflowLongFadeMA.LongFadeMAConfig.BucketUs = bucketUs }
    let engine = OrderflowLongFadeMA.Engine(cfgWithBucket)
    let mutable barCount = 0
    let mutable startUs = 0L
    let mutable endUs = 0L
    let mutable hasAny = false
    ...
    let reportingCfg =
        { defaultConfig cfg.CoverMaHours with
            Notional = cfg.Notional
            TakerFee = cfg.TakerFee
            AllowShort = false
            BucketUs = bucketUs
...
/// ShortFadeMA wrapper — mirror of LongFadeMACell. Short-only.
/// MaWindowHours field repurposed to carry the cover-MA window in hours.
type ShortFadeMACell(symbol: string, timeframe: string, cfg: OrderflowShortFadeMA.ShortFadeMAConfig) =
    let bucketUs = bucketUsOfTimeframe timeframe
    let builder = TimeBarBuilder(bucketUs)
    let cfgWithBucket = { cfg with OrderflowShortFadeMA.ShortFadeMAConfig.BucketUs = bucketUs }
    let engine = OrderflowShortFadeMA.Engine(cfgWithBucket)
    let mutable barCount = 0
    let mutable startUs = 0L
    let mutable endUs = 0L
    let mutable hasAny = false
    ...
    member _.Timeframe = timeframe
    member _.Config = reportingCfg
    member _.ShortFadeMAConfig = cfg
...
let runShortFadeMACellsFromBars
    (barsRoot: string)
    (symbol: string)
    (startDate: System.DateTime)
    (endDate: System.DateTime)
    (cells: ShortFadeMACell[])
    (fundingRoot: string option)
    : Metrics[] * float =
    let fundingEvents =
        match fundingRoot with
...
let runLongFadeMACellsFromBars
    (barsRoot: string)
    (symbol: string)
    (startDate: System.DateTime)
    (endDate: System.DateTime)
    (cells: LongFadeMACell[])
    (fundingRoot: string option)
    : Metrics[] * float =
    let fundingEvents =
        match fundingRoot with
...
// =============================================================================
// Donchian-fade cell + runner
// =============================================================================
//
// Donchian engine emits its own DonchianRoundTrip records (separate from the
// shared RoundTrip schema). For metrics we map the Donchian trips to a minimal
// RoundTrip[] internally — buildMetrics only reads NetPnL, EffectiveNotional,
// Side, BarsHeld, EntryUs, ExitUs, so the unused fields are zeroed.

type DonchianFadeCell(symbol: string, timeframe: string, cfg: OrderflowDonchianFade.DonchianFadeConfig) =
    let bucketUs = bucketUsOfTimeframe timeframe
    let builder = TimeBarBuilder(bucketUs)
    let cfgWithBucket =
        { cfg with OrderflowDonchianFade.DonchianFadeConfig.BucketUs = bucketUs }
    let engine = OrderflowDonchianFade.Engine(cfgWithBucket)
    let mutable barCount = 0
    let mutable startUs = 0L
    let mutable endUs = 0L
    let mutable hasAny = false

    // Reporting StrategyConfig — populates only the fields buildMetrics
    // and the per-cell results CSV inspect. MaWindowHours carries the
    // long-ref hours so the per-cell results row is identifiable.
    let reportingCfg =
        { defaultConfig cfg.LongRefHours with
            Notional = cfg.Notional
            TakerFee = cfg.TakerFee
            AllowShort = cfg.AllowShort
            BucketUs = bucketUs
    ...
    member _.Timeframe = timeframe
    member _.Config = reportingCfg
    member _.DonchianFadeConfig = cfg
    ...
    /// Map DonchianRoundTrip -> RoundTrip for shared-metrics reuse. The
    /// fields buildMetrics inspects (NetPnL, EffectiveNotional, Side,
    /// BarsHeld, EntryUs, ExitUs) are forwarded; the rest are zeroed.
    member _.BuildMetrics() =
        let donTrips = engine.Trips |> Seq.toArray
        let trips =
            donTrips
            |> Array.map (fun (t: OrderflowDonchianFade.DonchianRoundTrip) ->
                {
                    EntryUs = t.EntryUs
                    ExitUs = t.ExitUs
                    Side = t.Side
    ...
    /// Donchian-specific trip records (used by appendDonchianTrips).
    member _.Trips = engine.Trips |> Seq.toArray

let runDonchianFadeCellsFromBars
    (barsRoot: string)
    (symbol: string)
    (startDate: System.DateTime)
    (endDate: System.DateTime)
    (cells: DonchianFadeCell[])
    (fundingRoot: string option)
    : Metrics[] * float =
    let fundingEvents =
        match fundingRoot with
