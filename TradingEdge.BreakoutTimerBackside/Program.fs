module TradingEdge.BreakoutTimerBackside.Program

open System
open System.Diagnostics
open Argu
open TradingEdge.BreakoutTimerBackside.Intraday
open TradingEdge.BreakoutTimerBackside.Backtest
...
let private defaultCsv = "/tmp/breakout_timer_trips.csv"
...
type Args =
    ...
    | Require_Breakout_Timer
    | Breakout_Min_Bars_Since_High of int
    | Breakout_Timer_Bars of int
    | Min_Pb_Updn of float
    // ----- stop / exits -----
    | No_Geom_Stop
    | Stop_Floor_20m
    ...
    interface IArgParserTemplate with
        member s.Usage =
            ...
            | Min_Vol_Slope _ -> "LEGACY GATE (OFF by default, F13 dead weight): 20m OLS log-volume slope >= this."
            | Min_Vol_Climb _ -> "⭐ VOLUME GATE (F25): require vol_climb = (volEma-volEmaMin)/volEma >= this. Default 0.1."
            ...
            | Require_Breakout_Timer -> "FUSION GATE: permit the momentum entry ONLY while the breakout timer is live (>0). Default OFF = pure DipRiderV3."
            | Breakout_Min_Bars_Since_High _ -> "arm the breakout timer only after a >= N-bar 9-EMA-session-high drought. Default 8."
            | Breakout_Timer_Bars _ -> "the countdown length (bars) the momentum entry is permitted after a qualifying breakout. Default 10."
            | Min_Pb_Updn _ -> "pb_updn FLOOR (F8): reject if the pullback updn ratio < this (a monotone all-weather A+ lever at drought=16). 0 = off."
...
let main argv =
        ...
        { defaultConfig with
            Intraday =
              { defaultConfig.Intraday with
                  ...
                  RequireBreakoutTimer = parsed.Contains Require_Breakout_Timer || defaultConfig.Intraday.RequireBreakoutTimer
                  BreakoutMinBarsSinceHigh = parsed.GetResult(Breakout_Min_Bars_Since_High, defaultValue = defaultConfig.Intraday.BreakoutMinBarsSinceHigh)
                  BreakoutTimerBars = parsed.GetResult(Breakout_Timer_Bars, defaultValue = defaultConfig.Intraday.BreakoutTimerBars)
                  MinPbUpDn       = parsed.GetResult(Min_Pb_Updn, defaultValue = defaultConfig.Intraday.MinPbUpDn)
    ...
    printfn "BreakoutTimer backtest — 9-EMA-breakout + momentum-confirm (LONG intraday)"
    printfn "  db          = %s" dbPath
    ...
    printfn "  gates       = log-ATR20 >= %.3f   price-slope20 > %.3f   tightness20 >= %.1f%s%s%s"
        ic.MinAtrPct ic.MinPriceSlope ic.MinTightness
    ...
    let caps =
        [ if not (Double.IsInfinity ic.MaxVolSlope) then yield sprintf "vol-slope < %.2f" ic.MaxVolSlope
          ...
          if ic.MinPbUpDn > 0.0 then yield sprintf "pb_updn >= %.2f" ic.MinPbUpDn
    ...
    if ic.RequireBreakoutTimer then
        printfn "  breakout    = ON: momentum entry permitted only within %d bars of a new 9-EMA session high after a >= %d-bar drought"
            ic.BreakoutTimerBars ic.BreakoutMinBarsSinceHigh
    else
        printfn "  breakout    = OFF (pure DipRiderV3 momentum; --require-breakout-timer to enable the fusion gate)"
    let stopDesc =
        if ic.EmaStop then
            sprintf "EMA-stop (%s): level = 20m-min-9EMA (direct, no geometry); exit when live 9EMA < level"
                (if ic.EmaStopTrail then "TRAILING" else "FIXED@entry")
