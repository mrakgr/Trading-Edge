module TradingEdge.PlungeRider.Program

open System
open System.Diagnostics
open Argu
open TradingEdge.PlungeRider.Backtest
...
let private defaultOutDir = "/tmp/plungerider_trips"

/// PlungeRider CLI — the SHORT mirror of SurgeRider's. Deliberately TINY: this
/// is a SAMPLER; the study happens in SQL over the emitted trip PARQUET. If you
/// find yourself wanting a new GATE flag, add a recorded FEATURE instead.
type Args =
    ...
    interface IArgParserTemplate with
        member this.Usage =
            ...
            | Entry_Channel_Bars _ -> "⭐ ENTRY (SHORT): sell when the bar vwap UNDERCUTS the prior N-present-bar MIN of vwaps. One of {20,25,30,45,60,120,300,1200}. Low-breach counters for ALL windows are recorded — post-hoc can TIGHTEN to a longer window, not loosen."
            | Exit_Channel_Bars _ -> "STOP (opposite side): vwap ABOVE the prior N-present-bar MAX -> cover. One of {20,25,30,45,60,120,300,1200}."
            | Exit_Z_Bars _ -> "Which k-bar aggregate drives the acceleration-died exits. One of {1,15,30,60}. Default 60."
            | Ezv _ -> "Exit when z(ln k-bar volume sum vs the 1200-bar baseline) < this. Default -inf = off (F11)."
            | Ezt _ -> "Exit when z(ln k-bar trade-count sum) < this. Default -inf = off (F11)."
            | Dv_Floor_60 _ -> "Hard entry gate: >= this many DOLLARS traded over the trailing 60 present bars at the signal. Default 100000."
            | Tc_Floor_60 _ -> "Hard entry gate: >= this many TRADES over the same window. Default 60."
            | Min_Vol_20m _ -> "VOL FLOOR: vol_20m >= this at the signal (raw mean-|r|/30s units). Default 0.0007 (7bp) — ⚠ the LONG side's calibration. 0 = off."
...
[<EntryPoint>]
let main argv =
    let parser = ArgumentParser.Create<Args>(programName = "TradingEdge.PlungeRider")
    let parsed =
    ...
    if cfg.Intraday.EntryStartSec < 35100 then
        ...
        exit 1
    // Channel/z-window membership: the engine pre-builds exactly these windows.
    ...
    printfn "PlungeRider — 1s MOMENTUM (ride the %d-bar vwap-channel BREAKDOWN; SHORT)" ic.EntryChannelBars
    printfn "  db          = %s" dbPath
    ...
    printfn "  vol band    = vol_20m ∈ [%s, %s) bp/30s   ⚠ long-side calibration (floor 0 / ceiling 1e9 = off)"
        ...
        (if Double.IsPositiveInfinity ic.MaxVol20m || ic.MaxVol20m >= 1e6 then "inf" else sprintf "%.0f" (ic.MaxVol20m * 1e4))
    printfn "  EXIT (STOP) = z%d(ln vol) < %g  |  z%d(ln tc) < %g  |  vwap > prior %d-bar MAX  |  MOC   (fill: NEXT bar vwap)"
    ...
    if ic.MaxConcurrent <= 0 then
        printfn "  mode        = ⭐ SAMPLER (mc=0 unlimited → every breakdown bar opens another trip)"
    ...
    printfn "  net P&L    = %s   ⚠ costs not modeled  (SHORT: + = price fell)" ((stats.GrossWin - stats.GrossLoss).ToString "N0")
