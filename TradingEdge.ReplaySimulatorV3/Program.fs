...
let private dumpSnapshot (label: string) (s: Snapshot) =
    printfn ""
    printfn "===== play(%s) — %s ET =====" label (nyHmmssOf s.BucketStartNs)
    printfn "Venues with books: %d" s.Books.Count
    printfn "Trades in rolling 1m window: %d"
        (s.Trades |> Seq.length)
    printfn "Bars: %d" (List.length s.Bars)
    ...
    try
        ...
        if store.Records.Length > 0 then
            ...
            let runAt label hh mm ss =
                ...
                player.Play(min t lastTs) |> dumpSnapshot label
...
let main argv =
    try
        ...
        | _ ->
            ...
            printfn "  TradingEdge.ReplaySimulatorV3 scan  <day-dir>          -> print play(t) at a few times"
            1
    with ex ->
