...
let mergeTradesWithinThreshold (trades: JsonTrade[]) (thresholdNs: int64) : JsonTrade[] =
    if trades.Length = 0 then [||]
        ...
        for i in 1 .. trades.Length - 1 do
            let trade = trades.[i]
            if trade.participant_timestamp - group.[0].participant_timestamp < thresholdNs then
                group.Add(trade)
            else
...
let buildTDigestsFromJson (jsonPath: string) (compression: float) (mergeThresholdNs: int64) : TradeDataDigests =
    ...
    let merged = mergeTradesWithinThreshold filtered mergeThresholdNs
