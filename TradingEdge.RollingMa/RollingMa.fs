...
/// ⭐ How a DecaySumMa treats the gapCount missing seconds between pushes
/// (S35; user, 2026-08-29). The gap seconds are real time — the mode declares
/// what VALUE the quantity had during them, and the numerator and the weight
/// then update CONSISTENTLY (mixing conventions — e.g. a zero-filled sum over
/// an every-second weight — is the broken object that reads a "mean price"
/// below every price ever printed). With gapCount = 0 throughout, all three
/// modes coincide exactly.
type [<RequireQualifiedAccess>] GapValue =
    | Zero  // For volume and trade count. Assumes the gap values are 0.
    | Empty // For prices in bar-clock space. Assumes the gap values have no weight.
    | Locf  // For prices in time-clock space. Assumes the gap values are carried forward based on the last value. Last Order Carried Forward.

/// ⭐ Gap-aware DECAYED SUM + its normalized mean — THE gap-aware exponential
/// primitive (S8; user, 2026-08-27; GapValue modes S35, 2026-08-29). The SUM
/// is the primitive object, hl in TRADEABLE SECONDS (same gapCount convention
/// as TimeSumMa), every observation entering with weight 1. Per mode, a push
/// across g gap seconds (d = λ^(1+g)) does:
///   Zero:  s ← d·s + x            w ← d·w + (1−d)/(1−λ)   (= λ⁰+λ¹+…+λᵍ —
///          the g zeros are weight-1 observations; Mean = per-tradeable-second
///          RATE, the honest relvol denominator)
///   Empty: s ← d·s + x            w ← d·w + 1              (gaps decay but
///          carry no weight; Mean = decayed average of the PUSHES — on the
///          bar clock, gapCount = 0, this is the plain per-push EWMA)
///   Locf:  s ← d·s + xₚᵣₑᵥ·(λ−d)/(1−λ) + x                 (the gap seconds
///          hold the LAST value: Σₖ₌₁..ᵍ λᵏ·xₚᵣₑᵥ)
///                                 w ← d·w + (1−d)/(1−λ)    (every second
///          weighted; Mean = decayed TWAP of the LOCF-filled price path;
///          the FIRST push ignores its gapCount — no value to carry yet)
/// Mean = Sum / Weight is the exact bias-corrected EWMA of the declared path
/// (EmaHlMa's num = α·Sum and den = α·w, term for term, in Zero mode with no
/// gaps; the α-scaled class survives only as the frozen production surface).
///
...
/// zero at age 0 (the S8 EWMA speed). Numerator and Weight difference-pairs
/// stay consistent per mode, so (SumS−SumF)/(WeightS−WeightF) is the
/// prior-window mean of the declared path. ⚠ Only UNSCALED sums subtract this
/// way: with α-scaled pushes the age-0 weight of the difference is
...
/// (0, hl_slow/hl_fast].
[<Sealed>]
type DecaySumMa(halfLifeSecs: float, mode: GapValue) =
    ...
    /// The decayed sum of the declared path, or ValueNone before the first push.
    member _.Sum = if w > 0.0 then ValueSome s else ValueNone
    /// Sum / Weight — the bias-corrected exponentially weighted mean of the
    /// declared path (see the mode table above), or ValueNone before the first
    /// push. In Zero/Locf modes w -> 1/(1−λ) ≈ hl/ln2 + ½ as the support fills.
    member _.Mean = if w > 0.0 then ValueSome (s / w) else ValueNone
    /// The decayed total WEIGHT w under the declared mode (S35): Zero/Locf =
    /// Σ λ^age over every tradeable second; Empty = Σ λ^age over pushes only.
    /// The consistent denominator for window-difference means — oracle:
    /// DecaySumWeight_Test.fsx (the two weight families are 17× apart on
    /// sparse tape; identical on gap-0 streams).
    member _.Weight = if w > 0.0 then ValueSome w else ValueNone
    member _.Push (x: float, gapCount: int) =
        ...
        | GapValue.Empty ->
            s <- d * s + x
            w <- d * w + 1.0                       // pushes only; gaps decay weightlessly
        | GapValue.Locf ->
            if w = 0.0 then                        // first push: nothing to carry forward
                s <- x
                w <- 1.0
            else
                s <- d * s + last * (lam - d) / (1.0 - lam) + x   // gaps hold `last`
                w <- d * w + (1.0 - d) / (1.0 - lam)
            last <- x
    /// Clear the accumulator (see RollingMa.Reset).
    member _.Reset () =
...
// =============================================================================
// ZQuantMa — z-score QUANTILE COUNTERS (trend strength / convexity)
// =============================================================================
//
// ⭐ 2026-09-02 (user): count the bars whose |z| exceeds each of a ladder of
// thresholds, where z is the bar's own log-price against a trailing REFERENCE
// window. Two windows, deliberately separate: the Z-WINDOW (how z is defined)
// and the COUNT horizon (over how many bars the exceedances are tallied).
//
// ⭐ THE √3 WALL — the arithmetic that sets the ladder. For a PURE LINEAR RAMP
// (constant drift, zero noise) the z of the newest bar converges to
//     z_last = (n−1)·sqrt(3/(n(n+1)))  ->  sqrt 3 ≈ 1.7321   [SAMPLE sd, n−1]
// INDEPENDENT of how steep the drift is: z is scale-free, so doubling the drift
// doubles the numerator and σ alike and they cancel. A ramp over [0,1] has mean
// ½ and σ = 1/√12, hence (1−½)·√12 = √3. Consequences, measured 2026-09-02:
//   · ⚠ TWO DIFFERENT FRACTIONS — do not confuse them (oracle checks 4/4b):
//     (a) WITHIN ONE STATIC window, across its own positions i=0..n−1, the
//         fraction above t is max(0, 1 − t/√3): t=1.0 -> 42.3%, 1.5 -> 13.4%,
//         1.7 -> 1.9%, and EXACTLY ZERO for t >= √3.
//     (b) ACROSS ROLLING evaluation bars — which is what these counters tally —
//         a ramp gives EVERY warm bar the SAME z (the newest bar is always its
//         own window's extreme), so the fraction is 1 below z_last and 0 above.
//     The counters see (b). (a) is the shape law that explains the wall.
//   · So rungs BELOW the wall measure TREND STRENGTH; rungs ABOVE it can only
//     be reached by CONVEXITY (accelerating away from the window) or a JUMP —
//     they are an ANTI-trend signal. 1.7 is carried precisely to straddle it.
//   · ⚠ The wall binds POINTWISE, not on a rolling series. When z is recomputed
//     every bar against its OWN trailing window, a plain random walk puts 14.1%
//     of bars above |z|>2 and 1.3% above 3 (W=300) — each bar gets a fresh
//     window, so the tail is sampled repeatedly. Every rung is populated.
//
// ⭐ RATE, NOT COUNT, for the anchored (variable-span) counters. Measured on a
// random-walk null, the RATE is span-free across an 80× range of segment
// lengths (L = 60 -> 4800: t=1.0 rate 0.571 -> 0.565; t=2.0 0.159 -> 0.150;
// t=3.0 0.0125 -> 0.0124) while the raw count scales linearly (33 -> 2706).
// This is the structural difference from AnchoredEff, whose ratios DO drift
// with span (corr(eff9, slots) = −0.589): an efficiency ratio's numerator and
// denominator grow at different rates (√L vs L), whereas a threshold-crossing
// rate is a plain mean of indicators and the span cancels.
// ⚠ Span-free UNDER THE NULL is not span-free in the data — .Bars is exposed
// and MUST be recorded and conditioned on (the S43aj/ak lesson).
//
// Bar-clock by convention: this is a PRICE feature, and price features stay
// bar-clock in both engines (SpikeFader's pxDecayB* push gapCount 0).

/// The shared z-engine: owns ONE WinStdMa reference window and evaluates the
/// ladder once per bar. `Push` returns the per-rung exceedance indicators so a
/// caller can fan them into any number of accumulators without recomputing z.
/// `.Z` is the current bar's z (nan while the window is cold or degenerate).
[<Sealed>]
type ZLadder(zWindow: int, thresholds: float[]) =
    do if zWindow < 2 then invalidArg (nameof zWindow) "zWindow must be >= 2"
    let std = WinStdMa zWindow
    let ind = Array.zeroCreate<float> thresholds.Length
    let mutable z = nan
    member _.Thresholds = thresholds
    member _.ZWindow = zWindow
    /// The newest bar's z against the trailing window (INCLUSIVE of itself, so
    /// it matches the fixed-window z columns' convention). nan while cold.
    member _.Z = z
    /// Per-rung exceedance indicators (1.0 / 0.0) for the newest bar. All zero
    /// while z is nan — a cold window contributes nothing rather than faulting.
    member _.Indicators = ind
    /// Fold one bar's LOG PRICE. Push the value you want z measured on (the
    /// engine passes log vwap, so the feature is scale-invariant across a $0.80
    /// and a $40 stock — SpikeFader's universe has no price floor).
    member _.Push (logPx: float) =
        // z BEFORE the push would exclude the bar itself; the fixed-window z
        // columns include it, so push first and score against the fresh window.
        std.Push logPx
        z <- match std.Z logPx with ValueSome v -> v | ValueNone -> nan
        let a = abs z
        for i in 0 .. thresholds.Length - 1 do
            ind.[i] <- if Double.IsNaN z then 0.0 else (if a > thresholds.[i] then 1.0 else 0.0)
    member _.Reset () =
        std.Reset ()
        z <- nan
        Array.fill ind 0 ind.Length 0.0

/// ANCHORED exceedance counters: reset at an event (a new channel low), then
/// accumulate. Exposes both the raw COUNT and the span-free RATE, plus .Bars —
/// record the span and condition on it.
///
/// `minBars` is a fail-closed floor mirroring AnchoredEff's `slots < 3` guard:
/// a rate over a handful of bars is near-binary (at L=60 the t=3.0 rate is 0 or
/// 1/60 with nothing between), so .Rate reads nan until the span is credible.
[<Sealed>]
type AnchoredZCount(nThresholds: int, minBars: int) =
    let cnt = Array.zeroCreate<float> nThresholds
    let mutable bars = 0
    /// Bars folded since the anchor (⚠ RECORD THIS — see the type-level note).
    member _.Bars = bars
    member _.Count (i: int) = cnt.[i]
    /// count / bars, or nan below `minBars`.
    member _.Rate (i: int) = if bars < minBars then nan else cnt.[i] / float bars
    /// Fold the ladder's indicators for one bar.
    member _.Push (ind: float[]) =
        bars <- bars + 1
        for i in 0 .. cnt.Length - 1 do
            cnt.[i] <- cnt.[i] + ind.[i]
    member _.Reset () =
        Array.fill cnt 0 cnt.Length 0.0
        bars <- 0
